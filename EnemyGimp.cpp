...
void EnemyGimp::SetParam(void)
{
	//画像のファイル名
	imgFileName = "Gimp.png";
...
void EnemyGimp::KnockBack(void)
{

}

void EnemyGimp::SetStartPos(Vector2F pos)
{
}

void EnemyGimp::SetEndPos(Vector2F pos)
{
}

Vector2F EnemyGimp::GetSPos(void)
{
	return sPos_;
}

Vector2F EnemyGimp::GetEPos(void)
{
	return ePos_;
}

float EnemyGimp::GetTime(void)
{
	return time_;
}

Vector2F EnemyGimp::SetSPos(void)
{
	return sPos_;
}

Vector2F EnemyGimp::SetEPos(void)
{
	return ePos_;
}

float EnemyGimp::SetTime(void)
{
	return true;
}

void EnemyGimp::Move(void)
{
	step_ += 1.0f / 60.0f * moveRight_;
	float rate = step_ / time_;

	// 線形補間
	Vector2F posS2E = Vector2F(
		static_cast<float>(ePos_.x - sPos_.x)
		, static_cast<float>(ePos_.y - sPos_.y)
	);
	...
	pos_.x = sPos_.x + (posS2E.x * rate);
	pos_.y = sPos_.y + (posS2E.y * rate);
